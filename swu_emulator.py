...
DEFAULT_MNC = '456'
DEFAULT_APN = 'ims'
DEFAULT_IDR_TYPE_STR = 'ID_FQDN'
DEFAULT_TIMEOUT_UDP = 2
#DEFAULT_TIMEOUT_UDP_NAT_TRANSVERSAL = 2
DEFAULT_KEEPALIVE_SEND_INTERVAL = 20
DEFAULT_P_CSCF_IP4_ADDRESS_ATTRIBUTE = 20
DEFAULT_P_CSCF_IP6_ADDRESS_ATTRIBUTE = 21
...
P_CSCF_IP4_ADDRESS	               = DEFAULT_P_CSCF_IP4_ADDRESS_ATTRIBUTE
P_CSCF_IP6_ADDRESS	               = DEFAULT_P_CSCF_IP6_ADDRESS_ATTRIBUTE
FTT_KAT		                       = 22
...
DEFAULT_P_CSCF_IP4_ADDRESS_ATTRIBUTE = P_CSCF_IP4_ADDRESS
DEFAULT_P_CSCF_IP6_ADDRESS_ATTRIBUTE = P_CSCF_IP6_ADDRESS


class swu():

    def __init__(self, source_address,epdg_address,apn,idr_id_type,modem,default_gateway,mcc,mnc,imsi,pcscf_ip4_attribute,pcscf_ip6_attribute,ki,op,opc,netns):
        self.source_address = source_address
        self.epdg_address = epdg_address
        self.apn = apn
        self.idr_id_type = self.parse_idr_id_type_str(idr_id_type)
        self.com_port = modem
        self.default_gateway = default_gateway
        self.mcc = mcc
        self.mnc = mnc
        self.imsi = imsi
        self.pcscf_ip4_attribute = pcscf_ip4_attribute
        self.pcscf_ip6_attribute = pcscf_ip6_attribute
    ...
    def parse_idr_id_type_str(self, idr_id_type_str):
        return {
            'ID_FQDN': ID_FQDN,
            'ID_KEY_ID': ID_KEY_ID,
            # if unknown IDr, return ID_FQDN as a default
        }.get(idr_id_type_str, ID_FQDN)
    ...
    def set_variables(self):
        ...
        self.nat_keepalive_interval = DEFAULT_KEEPALIVE_SEND_INTERVAL
        self.state = 0
        ...
        if (self.idr_id_type == ID_FQDN):
            self.set_identification(IDR,ID_FQDN, self.apn)  # I've only ever seen this for ID_FQDN from my phone
        elif (self.idr_id_type == ID_KEY_ID):
            self.set_identification(IDR,ID_KEY_ID, str.encode(self.apn))
    ...
    def print_ikev2_decryption_table(self):
        with open(expanduser("~ethan/.config/wireshark/ikev2_decryption_table"), "a") as file_obj:
            print('IKEv2 DECRYPTION TABLE INFO (Wireshark):')
            # ~ethan/.config/wireshark/ikev2_decryption_table outgoing packets
            ...
            print(text)
            file_obj.write(text+'\n')
            # ~ethan/.config/wireshark/ikev2_decryption_table incoming packets
    ...
    def print_esp_sa(self):
        with open(expanduser("~ethan/.config/wireshark/esp_sa"), "a") as file_obj:
            print('ESP SA INFO (wireshark):')
            # ~ethan/.config/wireshark/esp_sa outgoing packets
            ...
            print(text)
            file_obj.write(text+'\n')
            # ~ethan/.config/wireshark/esp_sa incoming packets
    ...
    def send_data(self, data):
        """send data to localhost in addition to the intended ePDG. necessary for wireshark decryption ??"""
        if self.userplane_mode == ESP_PROTOCOL:
            #self.socket.sendto(data, ('127.82.73.1', 500))
            self.socket.sendto(data, self.server_address)
        else:
            #self.socket_nat.sendto(b'\x00'*4 + data, ('127.82.73.1', 4500))
            self.socket_nat.sendto(b'\x00'*4 + data, self.server_address_nat)

    def send_data_debug(self, data, address='127.82.74.1'):
        pass
        #"""send data to localhost to see data being constructed"""
        #if self.userplane_mode == ESP_PROTOCOL:
        #    self.socket.sendto(data, (address, 500))
        #else:
        #    self.socket_nat.sendto(b'\x00'*4 + data, (address, 4500))

    def send_nat_keepalive(self):
        while True:
            time.sleep(self.nat_keepalive_interval)
...
#######################################################################################################################
    ...
    def set_cp_list(self, cp_list):
         ...
         self.cp_list.append([int(self.pcscf_ip4_attribute)])
         self.cp_list.append([int(self.pcscf_ip6_attribute)])
...
### USER PLANE FUNCTIONS AND INTER PROCESS COMMUNICATION ####
    ...
    def set_routes(self):
        ...
        if self.ipv4_address_list != []:
            self.exec_in_netns("ip addr add " + self.ipv4_address_list[0] + "/32 dev " + self.tun_device)
            ...
            #self.exec_in_netns("route add -net 0.0.0.0/1 gw " + self.ipv4_address_list[0])
            #self.exec_in_netns("route add -net 128.0.0.0/1 gw " + self.ipv4_address_list[0])
        ...
        if self.dnsv4_address_list != [] or self.dnsv6_address_list != []:
            if self.netns_name:
                ...
                with open("/etc/netns/%s/resolv.conf" % self.netns_name, "w") as file_obj:
                    for i in self.dnsv4_address_list:
...
#### MESSAGES ####
    ...
    def create_IKE_AUTH_2(self):
        ...
        self.send_data_debug(header + b'\x00', '127.82.75.1')
        payload = self.encode_generic_payload_header(NONE,0,self.encode_payload_type_eap())
        self.send_data_debug(payload[4:], '127.82.75.2')
        packet = self.set_ike_packet_length(header+payload)
        self.send_data_debug(packet, '127.82.75.3')
...
#### STATES ####
                ...
                if i[0] == N:    #protocol_id, notify_message_type, spi, notification_data
                    ...
                    if i[1][0] in (EAP_REQUEST,) and i[1][2] in (EAP_AKA,):
                        if i[1][3] in (AKA_Challenge, AKA_Reauthentication):
                            ...
                            # these are values received from the ePDG
                            ...
                            if (RAND is not None and AUTN is not None) or (VECTOR is not None and ENCR_DATA is not None):
                                if RAND is not None and AUTN is not None:
                                        ...
                                        # this section is RFC 4187, section 9.4
                                        # not well tested code

                                        if len(self.RES) % 4 != 0:
                                            print("RES length is not a multiple of 4, adjusting")
                                            res_bytearray = bytearray(b'\x00' * (len(self.RES) % 4)) * bytearray(self.RES)
                                            self.RES = bytes(res_bytearray)

                                        eap_payload_response_len = len(bytes([EAP_RESPONSE]) + bytes([self.eap_identifier]) + bytes([RESERVED] * 2) + bytes([EAP_AKA]) + bytes([AKA_Challenge]) + bytes([RESERVED] * 2))
                                        eap_payload_response_len += len(bytes([AT_RES]) + bytes([2]) + bytes([RESERVED])) + (len(self.RES))
                                        eap_payload_response_len += len(bytes([AT_MAC]) + fromHex('05') + bytes([RESERVED] * 2)) + len(MAC) + 1

                                        eap_payload_response = bytes([EAP_RESPONSE]) + bytes([self.eap_identifier]) + struct.pack('!H',eap_payload_response_len) + bytes([EAP_AKA]) + bytes([AKA_Challenge]) + bytes([RESERVED] * 2)
                                        # start of at_res eap-aka attribute
                                        # TODO: check if calculation for AT_RES length is correct
                                        eap_payload_response += bytes([AT_RES]) + bytes([int(math.log2(len(self.RES)))]) + bytes([RESERVED]) + bytes([((len(self.RES)) + 0) * 8]) + self.RES
                                        # end of at_res, start of at_mac.
                                        #eap_payload_response += bytes([AT_MAC]) + fromHex('05') + bytes([RESERVED] * 2) + MAC
                                        eap_payload_response += bytes([AT_MAC]) + fromHex('05') + bytes([RESERVED] * 2) + bytes([RESERVED] * 16)


                                        # TODO: determine if this is AT_MAC in the EAP payload or if this is the IKEv2 integrity checksum
        ...
        if self.ike_decoded_header['exchange_type'] == IKE_AUTH and self.decoded_payload[0][0] == SK:
            ...
            for i in self.decoded_payload[0][1]:
                ...
                if i[0] == N:    #protocol_id, notify_message_type, spi, notification_data
                    ...
                    if i[1][0] == CFG_REPLY:
                        self.ipv4_address_list = self.get_cp_attribute_value(i[1][1], INTERNAL_IP4_ADDRESS)
                        self.dnsv4_address_list = self.get_cp_attribute_value(i[1][1], INTERNAL_IP4_DNS)
                        ...
                        self.pcscf4_address_list = self.get_cp_attribute_value(i[1][1], P_CSCF_IP4_ADDRESS)
                        self.pcscf6_address_list = self.get_cp_attribute_value(i[1][1], P_CSCF_IP6_ADDRESS)
                        print('IPV4 ADDRESS', self.ipv4_address_list)
                        print('DNS IPV4 ADDRESS', self.dnsv4_address_list)
                        print('IPV6 ADDRESS', self.ipv6_address_list)
                        print('DNS IPV6 ADDRESS', self.dnsv6_address_list)
                        print('P-CSCF IPV4 ADDRESS', self.pcscf4_address_list)
                        print('P-CSCF IPV6 ADDRESS', self.pcscf6_address_list)

                        if self.ipv4_address_list == [] and self.ipv6_address_list == []:
    ...
    def state_delete(self,initiator,kill = True):
        if initiator == True:
            ...
            self.delete_routes()
            self.nat_keepalive_sender_worker.terminate()
            if kill == True:
                exit(1)
    ...
    def state_connected(self):
        ...
        self.nat_keepalive_sender = multiprocessing.Pipe()
        ...
        self.nat_keepalive_sender_worker = multiprocessing.Process(target = self.send_nat_keepalive, args=([]))
        self.nat_keepalive_sender_worker.start()
...
#https functions
#ebm539: patched to http to avoid dealing with certificates
def https_imsi(server):
    r = requests.get('http://' + server + '/?type=imsi', verify=False)
    return r.json()['imsi']

def https_res_ck_ik(server, rand, autn):
    r = requests.get('http://' + server + '/?type=rand-autn&rand=' + rand + '&autn=' + autn, verify=False)
...
def main():
    ...
    cp_list = [
        ...
        [INTERNAL_IP6_DNS],
        [INTERNAL_IP6_DNS]
        # pcscf attributes are added in set_cp_list
        # yes it's jank, but the command line arguments are read *after*
    ...
    # the first dh in this list is the dh used for key exchange
    sa_list = [
    [
       [IKE,0],
    ...
    #   # these sa's should not be used IMO - the dh is 1024 bit
    #   # see Logjam - at least 2048 bit DH is recommended
    #[
    #   [IKE,0],
    #   [ENCR,ENCR_AES_CBC,[KEY_LENGTH,256]],
    #   [PRF,PRF_HMAC_SHA2_256],
    ...
    parser.add_option("-i", "--identification", dest="idr_id_type_str", default=DEFAULT_IDR_TYPE_STR, help="IDr ID type (ID_FQDN or ID_KEY_ID)")
    ...
    parser.add_option("--pcscf_ip4", dest="pcscf_ip4_attribute", default=DEFAULT_P_CSCF_IP4_ADDRESS_ATTRIBUTE, help="P-CSCF IPv4 address attribute")
    parser.add_option("--pcscf_ip6", dest="pcscf_ip6_attribute", default=DEFAULT_P_CSCF_IP6_ADDRESS_ATTRIBUTE, help="P-CSCF IPv6 address attribute")
    ...
    a = swu(options.source_addr,destination_addr,options.apn,options.idr_id_type_str,options.modem,options.gateway_ip_address,options.mcc,options.mnc,options.imsi,options.pcscf_ip4_attribute,options.pcscf_ip6_attribute,options.ki,options.op,options.opc,options.netns)
